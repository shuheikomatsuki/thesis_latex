...
クロスリンガル音声合成における発話内容の正確性，自然性，および話者類似性に与える影響を
...
LLMによるtext-to-token生成（テキストを，音声の特徴を離散的な記号列として
表現したtokenへ変換する処理）と条件付きflow matchingによる
token-to-speech生成（tokenを音響特徴へ変換する処理）から構成される
CosyVoice 3\cite{Du2025CosyVoice3}を主な分析対象とする．話者性の変化がどの処理に由来するかを
...
示すものではない．改善対象を特定した後は，対象モジュールへのLoRA
（Low-Rank Adaptation）などのパラメータ効率的な追加学習，または
adapterやprojector（話者埋め込みなどの特徴空間をモデルの条件付け空間へ
写像する層）を付加し，学習に用いるデータセットを決定する．

\section{データセット}

予備分析には，日本語と英語の音声を同一条件で比較するため，1名のバイリンガル
話者による日英パラレル音声を収録したJECS（Japanese-English Code-Switching
Speech Corpus）\cite{JECS}を用いる．JECSはプロンプト言語以外の話者差を
抑えられる一方，1話者のみであるため，得られた傾向を話者一般へ適用することは
できない．
...
提案手法のモデルに使用する学習データは，分析により更新対象を決定した後，話者数，
収録時間，録音品質，利用条件を考慮して選定する．学習を行う場合は，学習用，
検証用，評価用の話者を分離し，zero-shot評価の話者を学習データに含めない．

\section{評価方法}

既存のcross-lingual TTSモデル（CosyVoice 3など）と，
