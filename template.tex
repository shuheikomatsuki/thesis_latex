...
text-to-token生成部と，そのトークンおよび音声から抽出した話者特性を表すベクトルである
話者埋め込みを条件として
音響特徴へ変換する条件付きflow matchingによるtoken-to-speech生成部，
およびボコーダによる波形生成部から構成される．
% 話者性の変化がどの処理に由来するかを
...
本研究では，話者性の変化がこれらのどの処理に由来するかを分析する．
cross-lingual条件ではプロンプト音声のtokenをLLMの入力に用いず，
話者埋め込みはtoken-to-speech部の条件付けにのみ用いられる．そこで通常生成に
