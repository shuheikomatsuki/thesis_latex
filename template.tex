...
テキストと話者情報から音声特徴を離散トークンとして予測するLLMによる
text-to-token生成部と，そのトークンを条件付きflow matchingにより
音響特徴へ変換するtoken-to-speech生成部，およびボコーダによる波形生成部
から構成される．話者性の変化がどの処理に由来するかを
...
モデル内部に少量の学習可能な層を挿入するadapterや，話者埋め込みなどの
特徴空間をモデルの条件付け空間へ写像するprojectorを付加し，学習に用いる
データセットを決定する．
